...
// 学习打卡页面 - 以学习计划为主体
import { formatDate, getDateString } from '../../utils/date'
import { getStudyTemplates, saveStudyTemplates, StudyTemplate } from '../../utils/storage'

// 学习任务接口
interface StudyTask {
  id: string
  name: string
  timeSlot: string           // 学习时间段（如：08:00-09:00）
  studyCompleted: boolean    // 学习是否完成
  masteredCompleted: boolean // 掌握是否完成（掌握=完成，可获得积分）
  points: number             // 任务总积分
  earnedPoints: number       // 已获得积分（分段计算）
}
...
interface DailyStats {
  ...
  totalPoints: number       // 当天总积分
  earnedPoints: number      // 当天已获得积分
}
...
Page({
      ...
      if (selectedDate) {
        ...
        if (!isNaN(targetDate.getTime())) {
          this.setData({
            ...
            currentDateText: formatDate(targetDate, 'YYYY年MM月DD日')
          })
  ...
  initPage() {
    ...
    this.setData({
      currentDate,
      currentDateText: formatDate(currentDate, 'YYYY年MM月DD日')
    })
  },
  ...
  loadStudyPlans() {
    try {
      const dateKey = getDateString(this.data.currentDate)
      ...
      const studyPlans = savedPlans.map((plan: StudyPlan) => {
        // 确保每个任务都有完整的字段
        const completeTasks = plan.tasks.map((task: any) => ({
          id: task.id,
          name: task.name,
   ...
   * 计算计划统计数据
   */
  calculatePlanStats(plan: StudyPlan): StudyPlan {
   ...
   * 计算每日统计数据
   */
  calculateDailyStats(studyPlans: StudyPlan[]): DailyStats {
  ...
  onEditPlan(event: any) {
    ...
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm && res.content) {
          const studyPlans = this.data.studyPlans.map(p => 
            p.id === plan.id ? { ...p, name: res.content } : p
          )
          this.setData({ studyPlans })
          this.saveStudyPlans()
   ...
   * 保存学习计划为模板
   */
  onSavePlanAsTemplate(event: any) {
    const plan = event.currentTarget.dataset.plan
    ...
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm && res.content) {
          try {
            // 获取现有模板，确保是数组格式
            let templates = getStudyTemplates()
            ...
            const newTemplate: StudyTemplate = {
              id: Date.now().toString(),
              name: res.content,
              content: plan.tasks.map((task: StudyTask) => `${task.name}${task.timeSlot ? ` (${task.timeSlot})` : ''}`).join('\n'),
              createTime: new Date().toISOString(),
              tasks: plan.tasks.map((task: StudyTask) => ({
                name: task.name,
                timeSlot: task.timeSlot
              }))
            }
            ...
            // 保存模板
            saveStudyTemplates(templates.concat(newTemplate))
          ...
          const studyPlans = this.data.studyPlans.map(plan => {
            if (plan.id === planId) {
              const updatedTasks = plan.tasks.map(task => 
                task.id === taskId ? { ...task, name: res.content.trim() } : task
              )
              return this.calculatePlanStats({
                ...plan,
                tasks: updatedTasks
   ...
   * 显示时间段选择器
   */
  showTimeSlotSelector(planId: string, taskId: string) {
    // 第一级选择：时间段分类
    wx.showActionSheet({
      itemList: [
        '自定义时间段',
   ...
   * 显示上午时段选择
   */
  showMorningTimeSlots(planId: string, taskId: string) {
   ...
   * 显示下午时段选择
   */
  showAfternoonTimeSlots(planId: string, taskId: string) {
   ...
   * 显示晚上时段选择
   */
  showEveningTimeSlots(planId: string, taskId: string) {
   ...
   * 显示短时间段选择(30分钟)
   */
  showShortTimeSlots(planId: string, taskId: string) {
   ...
   * 显示长时间段选择(1.5-2小时)
   */
  showLongTimeSlots(planId: string, taskId: string) {
   ...
   * 显示自定义时间段输入框
   */
  showCustomTimeSlotInput(planId: string, taskId: string) {
    wx.showModal({
      title: '设置时间段',
   ...
   * 更新任务时间段
   */
  updateTaskTimeSlot(planId: string, taskId: string, timeSlot: string) {
    ...
    try {
      const studyPlans = this.data.studyPlans.map(plan => {
        if (plan.id === planId) {
          const updatedTasks = plan.tasks.map(task => 
            task.id === taskId ? { ...task, timeSlot } : task
          )
   ...
   * 更新任务状态
   */
  updateTaskStatus(planId: string, taskId: string, statusField: keyof StudyTask) {
    const studyPlans = this.data.studyPlans.map(plan => {
      if (plan.id === planId) {
        const updatedTasks = plan.tasks.map(task => {
          if (task.id === taskId) {
    ...
    try {
      ...
      wx.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            // 复制计划结构，重置掌握状态
            const copiedPlans = yesterdayPlans.map((plan: any) => ({
              ...plan,
            ...
            // 计算统计数据
            const studyPlans = copiedPlans.map((plan: StudyPlan) => this.calculatePlanStats(plan))
   ...
   * 将模板应用到今天
   */
  loadTemplateToToday(template: any) {
    try {
      let tasks: StudyTask[] = []
      
      // 优先使用模板的tasks结构（新格式）
      if (template.tasks && Array.isArray(template.tasks)) {
        tasks = template.tasks.map((taskTemplate: any, index: number) => ({
   ...
   * 解析模板内容为任务列表
   */
  parseTemplateContent(content: string): StudyTask[] {
   ...
   * 计算任务实际获得的积分
   */
  calculateTaskEarnedPoints(task: StudyTask): number {
    let earnedPoints = 0
