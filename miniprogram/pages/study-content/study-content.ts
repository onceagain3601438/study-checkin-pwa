...
Page<PageData, Record<string, any>>({
   ...
   * 初始化页面
   */
  initializePage(date: string, edit: any) {
  ...
  loadExistingData() {
    ...
    const existingRecord = getStudyRecordByDate(selectedDate);
  ...
  loadQuickActionData() {
    ...
    const yesterdayRecord = getStudyRecordByDate(yesterdayDate);
   ...
   * 选择模板项
   */
  onSelectTemplateItem(event: any) {
